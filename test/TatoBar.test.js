...
const TatoBar = artifacts.require('TatoBar');

contract('TatoToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        this.tato = await TatoToken.new({ from: alice });
        this.bar = await TatoBar.new(this.tato.address, { from: alice });
        this.tato.mint(alice, '100', { from: alice });
        this.tato.mint(bob, '100', { from: alice });
        this.tato.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.tato.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.tato.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.tato.approve(this.bar.address, '100', { from: alice });
        await this.tato.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.tato.balanceOf(this.bar.address)).valueOf(), '30');
        // TatoBar get 20 more TATOs from an external source.
        await this.tato.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more TATOs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.tato.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.tato.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.tato.balanceOf(bob)).valueOf(), '98');
    });
});
